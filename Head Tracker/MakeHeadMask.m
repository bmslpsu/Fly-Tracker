function [WingMask] = MakeHeadMask(vid)
% MakeWingMask:
    Vid = squeeze(vid);
    dim = size(Vid);
    figure
    ...
    rPts = [...
        impoint(gca, dim(2)*0.6, dim(1)*0.3, 'PositionConstraintFcn', @(pos) rPositionConstraintFcn(pos, dim))... 	% rTopRight
        impoint(gca, dim(2)*0.4, dim(1)*0.3, 'PositionConstraintFcn', @(pos) rPositionConstraintFcn(pos, dim))...  	% rBottomRight
        impoint(gca, dim(2)*0.5, dim(1)*0.5, 'PositionConstraintFcn', @(pos) rPositionConstraintFcn(pos, dim))...   % rInnerCircle
        impoint(gca, dim(2)*0.5, dim(1)*0.4,'PositionConstraintFcn', @(pos) rPositionConstraintFcn(pos, dim))]; 	% rCenter
